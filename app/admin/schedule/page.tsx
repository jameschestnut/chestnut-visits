  ...
  useEffect(() => {
    async function load() {
      ...
      const [
        ...
        supabase.from('visits').select(`id, school_id, visit_type, slot, visit_date, schools (id, name, short_name)`).eq('status', 'banked').lte('visit_date', (() => { const d = new Date(weekDates[4].dateStr); d.setDate(d.getDate() + 7); return toDateStr(d) })()),
