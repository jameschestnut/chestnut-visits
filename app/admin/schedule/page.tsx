...
// Alternating row colours — white and subtle green tint
const ROW_COLOURS = [
  ['#ffffff', '#f9fffe'],
  ['#f0fdf8', '#edfcf5'],
]
...
// Get display name — show surname initial if first name is shared with another technician
function getDisplayName(tech: Technician, allTechs: Technician[]): string {
  const firstName = tech.full_name.split(' ')[0]
  const surname   = tech.full_name.split(' ').slice(1).join(' ')
  const shared    = allTechs.filter(t => t.id !== tech.id && t.full_name.split(' ')[0] === firstName)
  if (shared.length > 0 && surname) {
...
export default function WeeklyPlannerPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [
        ...
        supabase.from('visits').select(`id, school_id, visit_type, slot, visit_date, schools (id, name, short_name)`).eq('status', 'banked').gte('visit_date', (() => { const d = new Date(weekStart); d.setDate(d.getDate() - 7); return toDateStr(d) })()).lte('visit_date', (() => { const d = new Date(weekEnd); d.setDate(d.getDate() + 7); return toDateStr(d) })()).order('visit_date'),
  ...
  function openPopover(techId: string, techName: string, dateStr: string, slot: string) {
    ...
    setPopVisitType('annual_leave'); setPopSchoolId(''); setPopNotes(''); setPopSchoolSearch(''); setPopAddToBank(false)
  }
    ...
    <div>
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0 bg-white rounded-xl border border-gray-100 overflow-x-auto">
          <table className="border-collapse text-xs" style={{ minWidth: 600, width: '100%' }}>
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="text-left px-2 py-2 font-medium text-gray-500 border-r border-gray-100 sticky left-0 bg-gray-50 z-10" style={{ minWidth: 100, width: 100 }}>Technician</th>
                <th className="px-1 py-2 text-gray-400 font-normal text-center border-r border-gray-100" style={{ width: 36 }}>Slot</th>
                {weekDates.map(d => {
            ...
            <tbody>
              {technicians.map((tech, ti) => {
                ...
                const displayName  = getDisplayName(tech, technicians)
                ...
                return SLOTS.map((slot, si) => (
                  <tr key={`${tech.id}-${slot}`} className="border-b border-gray-100 last:border-b-0"
                    ...
                    {si === 0 ? (
                      <td rowSpan={2} className="px-2 py-1 border-r border-gray-100 align-middle sticky left-0 z-10" style={{ background: amBg }}>
                        <Link href={`/admin/technicians/${tech.id}`} className="flex items-center gap-1.5 hover:opacity-75 transition-opacity">
                          {tech.photo_url ? (
                            <img src={tech.photo_url} alt={tech.full_name} className="w-6 h-6 rounded-full object-cover shrink-0" />
                          ) : (
                            <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0" style={{ background: BRAND_GREEN }}>{tech.initials}</div>
                          )}
                          <span className="font-semibold text-gray-800 truncate text-xs">{displayName}</span>
                        </Link>
                      </td>
                    ) : null}
                        ...
                        <td key={d.key} className="px-1 py-0.5 border-r border-gray-100 last:border-r-0"
                          ...
                          {(bh || !inTerm) && !visit ? (
                            <div className="rounded text-xs flex items-center justify-center"
                              style={{ minHeight: 24, background: bh ? '#fee2e220' : '#f1f5f920', border: `1px solid ${bh ? '#fca5a5' : '#cbd5e1'}`, color: bh ? '#b91c1c' : '#94a3b8' }}>
                              {bh ? '🏦' : '🏖️'}
                            </div>
                          ) : visit ? (
                            <div
                            ...
                            <div
                              ...
                              style={{ borderColor: isOver ? BRAND_GREEN : '#e2e8f0', background: isOver ? '#f0fdf4' : 'transparent', minHeight: 24 }}>
                              <span className="opacity-0 group-hover:opacity-50 text-gray-400 text-xs select-none">+</span>
                            </div>
                          )}
                        </td>
                      )
          ...
          {/* Legend */}
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-3 py-2 border-t border-gray-100 bg-gray-50">
            {VISIT_TYPES.filter(t => !t.isAbsence).map(t => (
              <div key={t.value} className="flex items-center gap-1 text-xs text-gray-500"><div className="w-2.5 h-2.5 rounded-sm" style={{ background: t.colour }} />{t.label}</div>
            ))}
            <div className="flex items-center gap-1 text-xs text-gray-500"><div className="w-2.5 h-2.5 rounded-sm border border-gray-300" style={{ background: 'repeating-linear-gradient(45deg,#94a3b8,#94a3b8 2px,transparent 2px,transparent 4px)' }} />Absence</div>
            <div className="flex items-center gap-1 text-xs text-gray-500">🏦 Bank holiday</div>
            <div className="flex items-center gap-1 text-xs text-gray-500">🏖️ Holiday</div>
          </div>
        </div>

        {/* Sidebar */}
        <div className="w-44 shrink-0 bg-white rounded-xl border border-gray-100 flex flex-col text-xs"
          ...
          <div className="px-3 py-2.5 border-b border-gray-100">
            ...
            <p className="text-gray-400 mt-0.5">{sidebar.length} to place</p>
          </div>
          <div className="mx-2 mt-2 rounded-lg border-2 border-dashed flex items-center justify-center transition-colors"
            style={{ minHeight: 36, borderColor: overSidebar ? BRAND_GREEN : '#e2e8f0', background: overSidebar ? '#f0fdf4' : 'transparent', color: overSidebar ? BRAND_GREEN : '#9ca3af' }}>
            <span className="text-xs px-2 text-center">{overSidebar ? 'Drop to bank' : 'Drag to bank'}</span>
          </div>
          <div className="px-2 py-2 space-y-1.5 flex-1 overflow-auto">
            {sidebar.length === 0
              ? <p className="text-gray-400 text-center py-3">All placed ✓</p>
              : sidebar.map(item => {
                const cfg = getVisitTypeConfig(item.visit_type)
                return (
                  <div key={item.id} draggable onDragStart={() => onDragStartSidebar(item)}
                    className="rounded-md px-2 py-1.5 text-white font-medium cursor-grab select-none"
          ...
          <div className="mx-2 mb-2 mt-1 rounded-lg border border-amber-200 bg-amber-50 overflow-hidden">
            <div className="px-2.5 py-2 border-b border-amber-100 flex items-center justify-between">
              <p className="text-xs font-semibold text-amber-800">
                {pending.length > 0 ? `${pending.length} pending` : 'No changes'}
              </p>
              {saved && <span className="text-xs text-green-600">✓ Saved</span>}
            </div>
            {pending.length > 0 && (
              <div className="px-2.5 py-1.5 space-y-0.5 max-h-24 overflow-auto">
                {pending.map((c, i) => <p key={i} className="text-xs text-amber-700 leading-snug">{c.label}</p>)}
              </div>
            )}
            <div className="px-2 pb-2 pt-1.5">
              <button onClick={handleConfirm} disabled={pending.length === 0 || saving}
                className="w-full py-1.5 rounded-md text-xs font-medium text-white disabled:opacity-40"
                style={{ background: pending.length > 0 ? '#374151' : '#9ca3af' }}>
          ...
          <div className="relative bg-white rounded-xl shadow-xl border border-gray-100 p-5 w-80" onClick={e => e.stopPropagation()}>
            ...
            {getVisitTypeConfig(popVisitType).needsSchool && (
              <div className="mb-4">
                ...
                <div className="max-h-28 overflow-auto border border-gray-100 rounded-lg">
            ...
            <div className="mb-4">
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input type="checkbox" checked={popAddToBank} onChange={e => setPopAddToBank(e.target.checked)} className="accent-gray-900" />
                <div>
                  <p className="text-xs font-medium text-gray-700">Add to visit bank</p>
                  <p className="text-xs text-gray-400 mt-0.5">Schedule later via the planner</p>
                </div>
              </label>
            </div>
            <div className="mb-4">
