        ...
        <div className="col-span-1 space-y-3">
          ...
          {(disrupted?.length ?? 0) > 0 && (
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              ...
              <div className="space-y-2">
                {disrupted!.slice(0, 3).map((v: { id: string; visit_date: string; schools: unknown; technicians: unknown }) => (
                  <div key={v.id} className="text-xs">
                    <p className="font-medium text-gray-900 truncate">{(v.schools as { name: string } | null)?.name}</p>
                    <p className="text-gray-400">{new Date((v as { visit_date: string }).visit_date + 'T12:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })} · {(v.technicians as { full_name: string } | null)?.full_name.split(' ')[0]}</p>
                  </div>
                ))}
              </div>
          ...
          {(lowFeedback?.length ?? 0) > 0 && (
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              ...
              <div className="space-y-2">
                {lowFeedback!.slice(0, 3).map((f: { id: string; rating: number; visits: unknown }) => (
                  <div key={f.id} className="text-xs">
                    <p className="font-medium text-gray-900 truncate">{((f.visits as { schools: { name: string } | null } | null))?.schools?.name}</p>
